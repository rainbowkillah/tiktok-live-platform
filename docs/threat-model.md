...
1. **Authentication/Authorization model**: **[RESOLVED]** A Role-Based Access Control (RBAC) model has been defined in [docs/rbac.md](rbac.md). It uses JWT-based authentication with `STREAMER`, `MODERATOR`, and `VIEWER` roles, enforcing stream-scoping on all sensitive endpoints.
...
| S-02 | Unauthenticated client calls API endpoints (rule creation, replay trigger) | B3 | Implement JWT-based RBAC model as defined in [docs/rbac.md](rbac.md); reject unauthenticated requests with HTTP 401 | Low — RBAC enforces strict auth checks |
| S-03 | Replay request spoofs a different user's session | B3 | Enforce stream-scoping and ownership checks in [docs/rbac.md](rbac.md); ensure the `streamId` in the JWT matches the session's streamId | Low — RBAC prevents cross-stream replay |
...
| I-04 | SSE stream (`/streams/:streamId/events`) accessed without authorization | B3 | Require authentication on SSE endpoint; enforce stream-scoping as defined in [docs/rbac.md](rbac.md) | Low — RBAC prevents unauthorized feed access |
| I-05 | User PII retained beyond the configured retention period | Storage | Daily pruning job (storage.md §4.1) enforces all six table retention policies: `events` and `actions_log` hard-deleted at 90 days (`EVENT_RETENTION_DAYS` / `ACTIONS_LOG_RETENTION_DAYS`); `sessions` and `streams` hard-deleted at 1 year (`SESSION_RETENTION_DAYS` / `STREAM_RETENTION_DAYS`); `users` pseudonymized (SHA-256 hash of `tiktok_user_id`/`unique_id`, null `display_name`/`avatar_url`) after 1 year of inactivity (`USER_RETENTION_DAYS`) — see storage.md §4.1 and §4.2; `rules` retained indefinitely (no PII) | Low if pruning job is operational; Medium if job fails silently |
...
| ID | Threat | Trust Boundary | Proposed Mitigation | Residual Risk |
|----|--------|---------------|---------------------|---------------|
| E-01 | Unauthenticated user creates, modifies, or deletes rules | B3 | Implement RBAC as defined in [docs/rbac.md](rbac.md); restrict rule mutations to `STREAMER` role only | Low — RBAC strictly gates rule management |
| E-02 | Rule engine executes a `moderate` (ban/timeout) action triggered by an unauthorized user | Rule Engine | `moderate` actions are gated by `MODERATOR` or `STREAMER` roles per [docs/rbac.md](rbac.md); log all moderation actions to `actions_log` | Low — RBAC ensures only authorized roles trigger moderation |
